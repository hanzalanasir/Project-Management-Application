...
- [ ] T017 **Verify (do not recreate) the shared `ETagExtensions`** at `src/ProjectManagementApp.Api/Common/ETagExtensions.cs` — confirms it writes the `xmin` row version as a strong `ETag`, reads/parses `If-Match`, and returns **400** when required but absent. **Created and unit-tested by 001 (T115/T122)**, not by this feature (research R-2's 2026-08-06 note, R-15 in 001's research.md) — this was originally a creation task; corrected when 001's own Admin user-management endpoints needed this machinery first and became its owner
