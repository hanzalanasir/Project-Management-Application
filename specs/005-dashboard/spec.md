...
- `Dashboard:OverdueBoundary` — the date **comparison** for "before today" (default: strictly before). *(The **timezone is fixed to UTC** and is deliberately **not** a config knob — see Assumptions; making it configurable would let a deployment break 006's NFR-002 parity requirement.)*
...
- **Overdue** uses 003's `due_date` and treats `Done` as the only terminal/completion status; **"before today" is evaluated in UTC — fixed, not configurable.** This matches 006's Clarifications (UTC, fixed, for cross-viewer reproducibility) and is what makes 006's **NFR-002 value-parity requirement satisfiable**: a configurable timezone here would let a deployment break a stated cross-feature requirement by configuration, since the same caller's overdue count would differ between the Dashboard tile and the Project Progress report. The date *comparison* (strictly before today) remains configurable; the *timezone* does not. Any additional field a richer metric might need (e.g. a task completion timestamp for time-to-close) that 002/003 did not commit to is an **assumption**, and such a metric is deferred to OQ-005-01 / 006 rather than forcing a schema change.
